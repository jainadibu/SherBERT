...
%[~,datasub] = datasample(cardiotitles2.Impact,70000);
%data = cardiotitles2(datasub,:);
data = card_final;
head(data)
...
% to categorical.
%data.Category = categorical(data.Urgency);
data.Category = categorical(data.CitingWorksCount>=median(data.CitingWorksCount));
%data.Category = categorical(data.logCWC >= 3.5840);
%data.Category = data.CitingWorksCount;
%data.Category = categorical(data.cranksim >=58);
%data.Category = categorical(data.msm>=1);
...
%data.Tokens = encode(tokenizer, data.Title);
data.Tokens = encode(tokenizer, join([data.Title,data.abstract],". "));
%%
...
reportsNew = [ ...
    "Utilizing Coronary Angiograms to Predict Patient Outcomes using Contrastive Learning"];
...
% TTrain = categorical(cardiotitles.msm(cardiotitles.abstract~="")>=1);
% dsXTrain = arrayDatastore(tokensNew,"OutputType","same");
% dsTTrain = arrayDatastore(TTrain);
% cdsTrain = combine(dsXTrain,dsTTrain);
% miniBatchSize = 32;
% paddingValue = mdl.Tokenizer.PaddingCode;
...
% end
% 
% featuresNew = featuresTrain';
% labelsNew = predict(net,featuresNew);

YScores = predict(net,featuresValidation);
[a,b,c,d,e] = perfcurve(TValidation,YScores(:,2),'true');
figure
...
plot(e(1),e(2),'bo');
t = c((a==e(1))&(b==e(2)));
title(['AUC: ',num2str(d),' Threshold: ',num2str(t)])
xlabel('False Positive Rate')
ylabel('True Positive Rate')

[a,b,c,d,e] = perfcurve(TValidation,YScores(:,2),'true','XCrit','reca','YCrit','prec');
figure
plot([0:0.1:1],[0:0.1:1]);
hold on
plot(a,b,'r');
...
% pair. Using mean pooling of token as opposed to CLS
